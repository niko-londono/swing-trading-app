...
const PIE_COLORS = ["#00ff88","#ffd700","#4aaeff","#ff6b6b","#aa88ff","#ff8c00","#00e5ff","#ff4da6","#c8ff00","#ff9d5c","#7effb2","#c084fc"];

// ── Data helpers ─────────────────────────────────────────────────────
const uid = () => Math.random().toString(36).slice(2, 9);

const emptyYear = () =>
  Array(12).fill(null).map(() => ({ trading: "", capital: "", accionesDetail: [] }));

// Seed: convert old acciones values to generic dividendo entries
const SEED_2026 = [
  { trading: 54.84, capital: 500,  accionesDetail: [{ id:"s1", tipo:"dividendo", ticker:"—", monto:0.64 }] },
  { trading: 0,     capital: 600,  accionesDetail: [] },
  { trading: 0,     capital: 800,  accionesDetail: [{ id:"s2", tipo:"dividendo", ticker:"—", monto:1.41 }] },
  { trading: 24.78, capital: 1000, accionesDetail: [{ id:"s3", tipo:"dividendo", ticker:"—", monto:21.15 }] },
  ...Array(8).fill(null).map(() => ({ trading: "", capital: "", accionesDetail: [] })),
];
...
const tipoBadgeColor = (tipo) => tipo === "dividendo" ? "#ffd700" : "#4aaeff";
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"#000000cc", display:"flex", alignItems:"flex-end", zIndex:200, backdropFilter:"blur(4px)" }}>
      <div onClick={e => e.stopPropagation()} style={{ width:"100%", maxWidth:"480px", background:"#111c1c", borderTop:"1px solid #00ff8844", borderRadius:"20px 20px 0 0", padding:"24px 20px 40px", boxSizing:"border-box" }}>
        ...
        {[
          { label:"TICKER", val:ticker, set:setTicker, type:"text",   ph:"AAPL" },
          { label:"CANTIDAD DE ACCIONES", val:shares, set:setShares, type:"number", ph:"10" },
          { label:"PRECIO / ACCIÓN ($)", val:price, set:setPrice, type:"number", ph:"150.00" },
        ].map(({ label, val, set, type, ph }) => (
          <div key={label} style={{ marginBottom:"12px" }}>
            <div style={labelSt}>{label}</div>
            <input type={type} placeholder={ph} value={val} onChange={e => set(e.target.value)} style={inputSt} />
          </div>
        ))}
        {valid && (
          <div style={{ background:"#071210", borderRadius:"10px", padding:"10px 14px", marginBottom:"14px", fontSize:"12px", color:"#c9c0b4" }}>
            Valor total: <span style={{ color:"#00ff88", fontWeight:"700" }}>${(parseFloat(shares)*parseFloat(price)).toFixed(2)}</span>
          </div>
        )}
        <div style={{ display:"flex", gap:"10px" }}>
          <button onClick={onClose} style={{ flex:1, padding:"15px", background:"#1a2a2a", border:"none", borderRadius:"12px", color:"#d4ccbf", fontSize:"13px", fontFamily:"inherit", cursor:"pointer" }}>Cancelar</button>
          <button disabled={!valid} onClick={() => { onSave({ ticker:ticker.toUpperCase().trim(), shares:parseFloat(shares), price:parseFloat(price) }); onClose(); }}
...
// ── Add Transaction Modal (for G/L Acciones detail) ──────────────────
function AddTransactionModal({ onSave, onClose }) {
  const [tipo,     setTipo]     = useState("dividendo");
  const [ticker,   setTicker]   = useState("");
  const [monto,    setMonto]    = useState("");       // dividendo
  const [sharesV,  setSharesV]  = useState("");       // venta
  const [pCompra,  setPCompra]  = useState("");       // venta
  const [pVenta,   setPVenta]   = useState("");       // venta
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"#000000cc", display:"flex", alignItems:"flex-end", zIndex:200, backdropFilter:"blur(4px)" }}>
      <div onClick={e => e.stopPropagation()} style={{ width:"100%", maxWidth:"480px", background:"#111c1c", borderTop:"1px solid #ffd70044", borderRadius:"20px 20px 0 0", padding:"22px 20px 40px", boxSizing:"border-box" }}>
        <div style={{ fontSize:"9px", letterSpacing:"3px", color:"#ffd700", marginBottom:"16px" }}>AGREGAR TRANSACCIÓN</div>

        {/* Tipo toggle */}
        <div style={{ display:"flex", gap:"8px", marginBottom:"16px" }}>
          {["dividendo","venta"].map(t => (
            <button key={t} onClick={() => setTipo(t)} style={{ flex:1, padding:"10px", background: tipo===t ? (t==="dividendo" ? "#2a2200" : "#001a2a") : "#0a1010", border:`1px solid ${tipo===t ? tipoBadgeColor(t)+"88" : "#1a2a2a"}`, borderRadius:"10px", color: tipo===t ? tipoBadgeColor(t) : "#9e968f", fontSize:"11px", fontFamily:"inherit", cursor:"pointer", fontWeight: tipo===t ? "700" : "400", letterSpacing:"1px", textTransform:"uppercase" }}>
            ...
            {ganancia !== null && (
              <div style={{ background: ganancia >= 0 ? "#071210" : "#1a0707", borderRadius:"10px", padding:"12px 14px", marginBottom:"14px" }}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:"8px", textAlign:"center" }}>
                  <div>
                    <div style={{ fontSize:"7px", color:"#9e968f", letterSpacing:"1px", marginBottom:"4px" }}>INVERTIDO</div>
                    <div style={{ fontSize:"13px", color:"#c9c0b4", fontWeight:"700" }}>${invertido?.toFixed(2)}</div>
                  </div>
                  <div>
                    <div style={{ fontSize:"7px", color:"#9e968f", letterSpacing:"1px", marginBottom:"4px" }}>G/L</div>
                    <div style={{ fontSize:"13px", color: ganancia >= 0 ? "#00ff88" : "#ff4455", fontWeight:"700" }}>{fmt(ganancia)}</div>
                  </div>
                  <div>
                    <div style={{ fontSize:"7px", color:"#9e968f", letterSpacing:"1px", marginBottom:"4px" }}>REND.</div>
                    <div style={{ fontSize:"13px", color: ganPct >= 0 ? "#00ff88" : "#ff4455", fontWeight:"700" }}>{ganPct !== null ? `${ganPct.toFixed(2)}%` : "—"}</div>
                  </div>
                </div>
              </div>
            )}
...
export default function App() {
  ...
  const [modal, setModal]             = useState(null);        // { i, field, label }
  const [addStock, setAddStock]       = useState(false);
  const [addTx, setAddTx]             = useState(null);        // monthIndex for new transaction
  ...
  // ── Transaction helpers ────────────────────────────────────────
  const addTransaction = (monthIdx, tx) => {
    setAllData(prev => {
  ...
  const computed = data.map(row => {
    const t  = row.trading === "" ? null : +row.trading;
    const c  = row.capital === "" ? null : +row.capital;
    const a  = (row.accionesDetail||[]).reduce((s,d) => s + (d.monto||0), 0);
    const hasActivity = t !== null || a > 0;
    const total   = hasActivity ? (t ?? 0) + a : null;
    const rendPct = (t !== null && c) ? (t / c) * 100 : null;
    return { ...row, total, rendPct, t, a, c };
  });
  ...
  const chartData = computed.map((r,i) => ({
    m: MONTHS_SHORT[i],
    acum: computed.slice(0,i+1).reduce((s,x) => s + (x.total??0), 0),
  }));

  // ── Portfolio helpers ──────────────────────────────────────────
  const totalPortfolioValue = portfolio.reduce((s,s2) => s + s2.shares * s2.price, 0);
  const pieData = portfolio.map(s => ({ name:s.ticker, value:parseFloat((s.shares*s.price).toFixed(2)) }));
  const barData = portfolio.map(s => ({ ticker:s.ticker, valor:parseFloat((s.shares*s.price).toFixed(2)) }));
  ...
  // All acciones transactions across current year (for Resumen history)
  const txHistory = computed
    .map((r,i) => ({ mes:MONTHS[i], mesIdx:i, txs: r.accionesDetail||[] }))
  ...
  const exportExcel = () => {
    ...
    Object.entries(allData).sort().forEach(([yr, rows]) => {
      const sheetData = [["MES","G/L TRADING","G/L ACCIONES","CAPITAL","DETALLE ACCIONES"]];
      rows.forEach((r,i) => {
        const accionesSum = (r.accionesDetail||[]).reduce((s,d) => s + (d.monto||0), 0);
        const detalle = (r.accionesDetail||[]).map(d =>
          d.tipo==="dividendo"
            ? `DIVIDENDO ${d.ticker} $${d.monto}`
            : `VENTA ${d.ticker} x${d.sharesVendidas} compra$${d.precioCompra} venta$${d.precioVenta} G/L$${d.monto}`
        ).join(" | ");
        sheetData.push([MONTHS[i], r.trading===""?"":r.trading, accionesSum||"", r.capital===""?"":r.capital, detalle]);
      });
      const ws = XLSX.utils.aoa_to_sheet(sheetData);
      ws["!cols"] = [{wch:14},{wch:14},{wch:14},{wch:12},{wch:60}];
      XLSX.utils.book_append_sheet(wb, ws, `Trading_${yr}`);
    });
    const portData = [["TICKER","ACCIONES","PRECIO","VALOR TOTAL"],
    ...
    wsP["!cols"] = [{wch:10},{wch:12},{wch:12},{wch:14}];
      ...
      try {
        ...
        wb.SheetNames.forEach(name => {
          if (name.startsWith("Trading_")) {
            const yr = parseInt(name.replace("Trading_",""));
            if (isNaN(yr)) return;
            ...
            for (let r = 1; r <= 12; r++) {
              ...
              const detalle = [];
              if (row[4]) {
                String(row[4]).split(" | ").forEach(part => {
                  if (part.startsWith("DIVIDENDO")) {
                    const m = part.match(/DIVIDENDO (\S+) \$([0-9.]+)/);
                    if (m) detalle.push({ id:uid(), tipo:"dividendo", ticker:m[1], monto:parseFloat(m[2]) });
                  } else if (part.startsWith("VENTA")) {
                    const m = part.match(/VENTA (\S+) x([0-9.]+) compra\$([0-9.]+) venta\$([0-9.]+) G\/L\$([0-9.-]+)/);
                    if (m) detalle.push({ id:uid(), tipo:"venta", ticker:m[1], sharesVendidas:parseFloat(m[2]), precioCompra:parseFloat(m[3]), precioVenta:parseFloat(m[4]), monto:parseFloat(m[5]) });
                  }
                });
              }
              months.push({ trading:row[1]!==undefined&&row[1]!==""?parseFloat(row[1]):"", capital:row[3]!==undefined&&row[3]!==""?parseFloat(row[3]):"", accionesDetail:detalle });
            }
          ...
          if (name === "Portafolio") {
            ...
            for (let r = 1; r < rows.length; r++) {
              const row = rows[r];
              if (row[0]&&row[1]&&row[2]) newPortfolio.push({ ticker:String(row[0]), shares:parseFloat(row[1]), price:parseFloat(row[2]) });
            }
          }
        });
...
Da análisis crítico en 4 puntos concisos con emoji. Español directo.`;
    ...
    setAiLoading(false);
  }, [computed, ytd, faltante, promedio, necesario, activeYear]);
    ...
    <div style={{ flex:1, overflowY:"auto", padding:"16px" }}>
      <div style={{ background:"linear-gradient(135deg,#0a1f12,#071a1a)", border:"1px solid #00ff8820", borderRadius:"18px", padding:"20px", marginBottom:"12px" }}>
        ...
        <div style={{ fontSize:"10px", color:"#d4ccbf", marginTop:"4px" }}>
          de{" "}
          <span onClick={() => setEditGoal(true)} style={{ color:"#ffd700", borderBottom:"1px dashed #ffd70066", cursor:"pointer" }}>
            ${goal}
          </span>
          {" "}meta anual <span style={{ color:"#ffd70055", fontSize:"8px" }}>✎</span>
        </div>
  ...
  // ── Transaction card (shared between Tabla and Resumen) ────────
  const TxCard = ({ tx, onRemove }) => {
    ...
    const badgeCol  = tipoBadgeColor(tx.tipo);
    return (
      <div style={{ background:"#080f0f", borderRadius:"10px", padding:"10px 12px", marginBottom:"6px", borderLeft:`2px solid ${badgeCol}66` }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
            <span style={{ fontSize:"11px", fontWeight:"700", color:"#fff" }}>{tx.ticker}</span>
            <span style={{ fontSize:"7px", letterSpacing:"1px", color:badgeCol, background:`${badgeCol}18`, padding:"2px 6px", borderRadius:"4px" }}>
              {tx.tipo.toUpperCase()}
            </span>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
            <span style={{ fontSize:"13px", fontWeight:"700", color: tx.monto >= 0 ? "#00ff88" : "#ff4455" }}>{fmt(tx.monto)}</span>
            {onRemove && (
              <button onClick={onRemove} style={{ background:"none", border:"none", color:"#ff445566", fontSize:"12px", cursor:"pointer", padding:"0 2px" }}>✕</button>
            )}
          </div>
        </div>
        {tx.tipo === "venta" && (
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"4px", marginTop:"8px" }}>
            {[
              { label:"ACCIONES", val:`${tx.sharesVendidas}` },
              { label:"COMPRA",   val:`$${tx.precioCompra}` },
              { label:"VENTA",    val:`$${tx.precioVenta}` },
              { label:"REND.",    val: ganPct !== null ? `${ganPct.toFixed(1)}%` : "—" },
            ].map(({ label, val }) => (
              <div key={label} style={{ textAlign:"center" }}>
                <div style={{ fontSize:"6px", color:"#9e968f", letterSpacing:"1px" }}>{label}</div>
                <div style={{ fontSize:"10px", color:"#c9c0b4", fontWeight:"600", marginTop:"2px" }}>{val}</div>
              </div>
            ))}
          </div>
        )}
          ...
          <div key={i} style={{ background:"#0c1318", border:`1px solid ${hasData?col+"33":"#1a2a2a"}`, borderRadius:"14px", marginBottom:"8px", overflow:"hidden", opacity:hasData?1:0.55 }}>
            ...
            <div onClick={() => setExpanded(isOpen?null:i)} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 16px", cursor:"pointer" }}>
              <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                ...
                {txs.length > 0 && <span style={{ fontSize:"7px", color:"#ffd70099", letterSpacing:"1px" }}>{txs.length} tx</span>}
              </div>
            ...
            {isOpen && (
              <div style={{ borderTop:"1px solid #1a2a2a", padding:"14px" }}>
                {/* Trading + Capital cells */}
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px", marginBottom:"10px" }}>
                  {[{ field:"trading",label:"G/L TRADING" },{ field:"capital",label:"CAPITAL" }].map(({ field,label }) => (
                    <div key={field} onClick={() => setModal({ i, field, label })} style={{ background:"#0a1818", border:"1px solid #1a2a2a", borderRadius:"10px", padding:"10px", cursor:"pointer", textAlign:"center" }}>
                      ...
                      <div style={{ fontSize:"7px", color:"#00ff8866", marginTop:"4px" }}>✎ editar</div>
                    </div>
                  ))}
                </div>

                {/* G/L Acciones section */}
                <div style={{ background:"#0a1010", borderRadius:"10px", padding:"12px", marginBottom:"10px" }}>
                  ...
                  {txs.length === 0 ? (
                    ...
                    txs.map(tx => (
                      <TxCard key={tx.id} tx={tx} onRemove={() => removeTransaction(i, tx.id)} />
                    ))
                  )}
                </div>
                ...
                {/* Summary bar */}
                {row.total !== null && (() => {
                  const tradingPct = (row.t !== null && row.c) ? (row.t / row.c) * 100 : null;
                  return (
  ...
  const ResumenScreen = () => (
    <div style={{ flex:1, overflowY:"auto", padding:"16px" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#0d1a2e,#071a1a)", border:"1px solid #4af3", borderRadius:"18px", padding:"20px", marginBottom:"12px" }}>
        ...
        <div style={{ fontSize:"10px", color:"#c9c0b4", marginTop:"4px" }}>{portfolio.length} posición(es) activa(s)</div>
      </div>
      ...
      {portfolio.length > 0 && (
        <div style={{ background:"#0c1318", border:"1px solid #1a2a2a", borderRadius:"16px", padding:"16px", marginBottom:"12px" }}>
          <div style={{ fontSize:"8px", letterSpacing:"3px", color:"#c9c0b4", marginBottom:"10px" }}>VALOR POR ACCIÓN ($)</div>
      ...
      <div style={{ marginBottom:"16px" }}>
        ...
        {portfolio.map((s,idx) => {
          ...
          return (
            <div key={idx} style={{ background:"#0c1318", border:`1px solid ${col}33`, borderRadius:"14px", padding:"14px", marginBottom:"8px", borderLeft:`3px solid ${col}` }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                <div>
                  <div style={{ fontSize:"16px", fontWeight:"700", color:"#fff", letterSpacing:"1px" }}>{s.ticker}</div>
                  <div style={{ fontSize:"10px", color:"#c9c0b4", marginTop:"2px" }}>{s.shares} acciones · ${s.price.toFixed(2)}/u</div>
                </div>
                <div style={{ textAlign:"right" }}>
                  <div style={{ fontSize:"16px", fontWeight:"700", color:col }}>${valor.toFixed(2)}</div>
                  <div style={{ fontSize:"10px", color:"#c9c0b4", marginTop:"2px" }}>{weight.toFixed(1)}% del portafolio</div>
                </div>
              </div>
              <div style={{ marginTop:"10px", background:"#0a1818", borderRadius:"4px", height:"4px", overflow:"hidden" }}>
      ...
      {/* ── Historial de Acciones (linked from Tabla) ── */}
      <div>
        <div style={{ fontSize:"8px", letterSpacing:"2px", color:"#c9c0b4", marginBottom:"10px" }}>
          HISTORIAL DE ACCIONES · {activeYear}
        </div>
        {txHistory.length === 0 ? (
          <div style={{ background:"#0c1318", border:"1px dashed #1a2a2a", borderRadius:"14px", padding:"20px", textAlign:"center" }}>
            <div style={{ fontSize:"10px", color:"#9e968f" }}>Sin transacciones registradas en {activeYear}</div>
            <div style={{ fontSize:"9px", color:"#9e968f", marginTop:"4px" }}>Agrégalas desde la pestaña TABLA</div>
          </div>
        ) : txHistory.map(({ mes, mesIdx, txs }) => {
          const totalMes = txs.reduce((s,t) => s + t.monto, 0);
          return (
  ...
  const AIScreen = () => (
    <div style={{ flex:1, overflowY:"auto", padding:"16px" }}>
      ...
      <div style={{ background:"#0c1318", border:"1px solid #1a2a2a", borderRadius:"16px", padding:"18px", marginBottom:"14px" }}>
        <div style={{ fontSize:"9px", letterSpacing:"3px", color:"#c9c0b4", marginBottom:"14px" }}>⇅ IMPORTAR / EXPORTAR</div>
        <div style={{ display:"flex", gap:"10px" }}>
  ...
  return (
    <div style={{ width:"100%", maxWidth:"480px", height:"100dvh", background:"#080d0f", display:"flex", flexDirection:"column", fontFamily:"'Courier New',monospace", overflow:"hidden", margin:"0 auto", position:"relative" }}>
      ...
      <div style={{ padding:"16px 20px 12px", borderBottom:"1px solid #0f1a1a", flexShrink:0 }}>
      ...
      {addStock && <AddStockModal onSave={s => setPortfolio(prev=>[...prev,s])} onClose={() => setAddStock(false)} />}
      {addTx !== null && <AddTransactionModal onSave={tx => addTransaction(addTx, tx)} onClose={() => setAddTx(null)} />}
